 ...
 */
/* jshint -W062 */
/* global MiluParams, WOW, AOS, WVC, WolfCore, Cookies, Event, objectFitImages, WVCBigText, WVCParams, MiluYTVideoBg, Vimeo */
	...
	return {
		...
		toggleMenu: function () {
			...
			$(document).on("click", ".toggle-offcanvas-menu", function (event) {
				event.preventDefault();
				$( window ).trigger("milu_offcanvas_menu_toggle_button_click", [
					$(this),
				]);
...
(function ($) {
	...
	if ("undefined" !== typeof WolfCore) {
		 ...
		 * Overwrite WVC lightbox function with the theme function
		 */
		WolfCore.lightbox = MiluUi.lightbox;

		/**
		 * Overwrite WolfCore video opener
		 */
		WolfCore.videoOpener = MiluUi.videoOpener;

		/**
		 * Overwrite toolbar offset calculation
		 */
		WolfCore.getToolBarOffset = MiluUi.getToolBarOffset;
	}
